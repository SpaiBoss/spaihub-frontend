...
export default function ContributorWallet() {
  ...
  const [form, setForm] = useState({
    amountXaf: '',
    phoneNumber: contributor?.momoPhone || '',
    method: 'MTN_MOMO',
  });
  ...
  async function loadWallet(currentPage = page) {
    try {
      setError(null);
      const { data } = await api.get(`/api/contributor/wallet?page=${currentPage}`);
      setWallet(data);
  ...
  function openWithdrawModal() {
    ...
    setForm((f) => ({
      ...f,
      phoneNumber: f.phoneNumber || contributor?.momoPhone || '',
    }));
  ...
  async function handleWithdraw(e) {
    ...
    try {
      const { data } = await api.post(
        '/api/contributor/wallet/withdraw',
        {
